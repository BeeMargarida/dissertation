...
The solution implemented was to use the Node-RED instance to orchestrate the decentralization and send tasks to other devices in the network, which Node-RED knows previously. To make this possible, Node-RED needs to communicate with these devices via WiFi to send tasks and check their state of operation. Due to the devices' limitations, they cannot run an instance of Node-RED, so Node-RED needs to convert tasks in Javascript to other language that can be interpreted by these devices. The implemented solution requires a Micropython port and a HTTP server that receives and executes a micropython script created by the Node-RED. This script contains the tasks assigned to a device, translated into micropython.

After orchestrating and assigning tasks to devices, Node-RED must keep track of the state of these devices. If one fails and becomes unaccessible, Node-RED must adapt and assign the failed device's task to another.
