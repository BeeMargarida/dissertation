...
\begin{table}[ht]
    ...
    \begin{tabular}{| l | r | r |}
        ...
        \hline
        Exclusion/Inclusion criteria (Introduction and Conclusion) & 30 & 35\\
        \hline
        Specificity &  & \\
        \hline
    \end{tabular}
...
After analyzing the 30 publications, we organized them by categories. \cite{pratical_trigger_action}, \cite{survey_vpl_iot} and \cite{block-based_vs_flow-based} were surveys and the remaining 27 were frameworks or tools.
...
\par The remaining 27 articles are frameworks or tools of visual programming applied to IoT. One of the tool is repeated in two papers, which showcases its evolution. The frameworks are:
...
\begin{enumerate}
    ...
    \item \textbf{WARBLE} \cite{warble} is a middleware for IoT applications that provides a seamless interoperability between heterogeneous devices. It allows applications to discover available devices across different vendors and domains and contains an interaction history that make it possible for IoT devices to make decisions based on previous interactions. Additionally, it introduces the binding concept, that connects a certain device to an application and simplifies the communication between them for the developer. The developed solution address challenges like interoperability, personalization and simplicity but still has several challenges left to address, like security and privacy.
    ...
    \item The approach on Runtime Verification proposed by \textbf{Leotta et al.}  aims to assure the quality of an IoT system. It can be used for detecting bugs in systems during development and maintenance activities, as well as be adopted for monitoring deployed IoT systems. Runtime verification is a software analysis approach in which a running system is observed by monitoring relevant events and their associated information to verify against a given specification of the expected behavior. When efficiently implemented, the verification process can be executed before the deployment of the system, for detecting bugs during the development and maintenance activities, as well as after the deployment, in order to provide an additional level of protection against unforeseen events.
    The results of the runtime verification tool developed were positive, with 92  \% detection of the bugs injected into the system. For future work, they plan to automate the generation of trace expressions and scenarios, as well as compare their approach with testing.
...
\textcolor{blue}{explicar caracteristicas de classificação; fazer o quadro com tudo organizado;}
