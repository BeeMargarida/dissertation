...
\begin{description}
    ...
    \item [Architecture] Visual programming tools applied to the Internet of Things can have an centralized or decentralized architecture, based on their use of Cloud, Fog or Edge Computing architecture. Possible values are \textit{Cloud}, \textit{Fog} and \textit{Edge}.
    ...
    \item [Paradigm] The visual programming language can be imperative or functional programming, which can also be called block-based and flow-based respectively. It is important to know which type, so that might be possible to assess the type of experience the tool provides to the user and its architecture. Possible values can be \textit{block-based} or \textit{flow-based}. \textcolor{red}{\textbf{CHECK THIS}}
...
\begin{table}[ht]
    \centering
    \caption{VPLs applied to IoT and their characteristics. Small circles (\textbullet) mean \textit{yes}, hyphens (-) mean \textit{no information available}, nothing means \textit{no} and asterisk (*) means more than one.}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{| c | c | c | c | c | c | c |}
        \hline
        \textbf{Tool} & \textbf{Scope} & \textbf{Architecture} & \textbf{License} & \textbf{Scalability} & \textbf{Paradigm} & \textbf{Web-based}\\
        \hline
        Belsa et al.\cite{Belsa2018} & * & Cloud & - & High & Flow-based & \textbullet \\
        \hline
        Ivy \cite{ivy} & * & Cloud & - & High & Flow-based &  \\
        \hline
        Ghiani et al. \cite{personalization_of_context_dependent_apps} & Home Automation & Cloud &  &  &  & \textbullet \\
        \hline
        ViSiT \cite{visit} &  &  &  &  &  &  \\
        \hline
