...
\textcolor{blue}{Falar sobre todos os artigos do Giang e a sua evolução (WIP); falar sobre ddflow; falar sobre fogflow}
\textcolor{red}{WIP}
...
\par The work made by Giang et al. in DDF \cite{ddf} and subsequent works \cite{fog_at_the_edge} \cite{exogenous_coordination} consists of a system built on Node-RED framework and focused on the use case of Smart Cities. In DDF \cite{ddf}, they started by extending Node-RED and implementing a tool called D-NR (Distributed Node-RED), where processes run across devices in local networks and servers in the Cloud. The application, called flow, is built in the visual programming environment which is running in a development server. All the other devices running D-NR subscribe to a MQTT topic that contains the status of the flow. \textcolor{red}{CONTINUE}  %https://sci-hub.se/10.1109/IOT.2015.7356560 
